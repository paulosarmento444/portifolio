...
async function loadAccountOverview(): Promise<AccountOverviewView | null> {
    ...
    if (sessionState.session.isAuthenticated && viewer) {
      const [cart, orders, wooCustomer] = await Promise.all([
        ...
        wordpressWooRestAdapter
          .getAccountCustomer(Number(viewer.id))
          .catch(() => null),
      ]);
      const customer =
        wooCustomer ||
        buildCartDerivedCustomer(viewer, cart?.customer || null);
