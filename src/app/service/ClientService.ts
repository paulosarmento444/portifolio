import { woocommerceClient1 } from "../lib/clients/wooCommerce";
...
export const searchProducts = async (
  ...
  try {
    const response = await woocommerceClient1.get("/products", {
      params: {
        search: name,
...
export const getCategories = async (): Promise<any> => {
  ...
  try {
    const response = await woocommerceClient1.get("/products/categories");
...
export const getProduct = async (id: number): Promise<any> => {
  ...
  try {
    const response = await woocommerceClient1.get(`/products/${id}`);
    setCachedData(cacheKey, response.data);
    return response.data;
  } catch (error) {
...
export const getProductVariation = async (id: number): Promise<any> => {
  ...
  try {
    const response = await woocommerceClient1.get(`/products/${id}/variations`);
    setCachedData(cacheKey, response.data);
    return response.data;
  } catch (error) {
...
export const getProductsByIds = async (ids: number[]): Promise<any[]> => {
  try {
    const productRequests = ids.map((id) =>
      woocommerceClient1.get(`/products/${id}`)
    );
